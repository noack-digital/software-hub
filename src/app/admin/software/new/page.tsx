...
export default function NewSoftwarePage() {
  ...
  const { data: targetGroups = [], isLoading: isLoadingTargetGroups } = useQuery<TargetGroup[]>({
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {targetGroups.map((targetGroup) => (
                    <div key={targetGroup.id} className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor={`targetGroup-${targetGroup.id}`} className="font-normal cursor-pointer">
                        {targetGroup.name}
                      </Label>
                    </div>
                  ))}
