...
export async function GET(request: NextRequest) {
  try {
    ...
    // Zielgruppenfilter hinzufügen, wenn vorhanden
    if (targetGroupId) {
      whereClause = {
        ...whereClause,
        targetGroups: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const formattedSoftware = {
      ...
      targetGroups: newSoftware.targetGroups.map(tg => ({
        id: tg.targetGroup.id,
        name: tg.targetGroup.name
      }))
    };
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Bestehende Kategorien und Zielgruppen löschen und neue erstellen
